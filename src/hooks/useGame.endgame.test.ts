  ...
  return {
    useQuackleContext: () => ({
      ...
      makeMove: vi.fn(async (_gameState: any, rack: any[]) => {
        // Place entire rack horizontally starting from row 7, col 0
        const tiles = rack.map((t, idx) => ({ row: 7, col: idx, letter: (t.letter || 'A').toString().toUpperCase(), points: t.points || 1, isBlank: !!t.isBlank }))
        return {
          tiles,
