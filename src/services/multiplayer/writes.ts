...
import { nextPlayerId as nextId, evaluateEndgameAfterMove, evaluateEndgameOnPass, winnerIdFromTag, MoveLogEntry, applyEndgame } from './helpers'
...
export async function submitMoveForGame(params: {
  ...
  // Next player
  const nextPlayerId = nextId(game)
  ...
  const endEval = evaluateEndgameAfterMove({
    player1Score: player1ScoreAfter,
    player2Score: player2ScoreAfter,
    player1Rack: player1RackAfter,
    player2Rack: player2RackAfter,
    remainingBag: remaining,
  })
  if (endEval.endGame) {
    // sync local vars to the final scores
    player1ScoreAfter = endEval.player1Score
    player2ScoreAfter = endEval.player2Score
    // apply endgame to update object (status, scores, winner)
    applyEndgame(game, gameUpdate, endEval)
  }
  ...
  const endSummary = applyEndgame(game, {}, endEval)
  return { endGame: endSummary.endGame, winnerId: endSummary.endGame ? endSummary.winnerId : undefined }
}
...
export async function passTurnForGame(params: {
  ...
  const endEval = evaluateEndgameOnPass({
    passCount: newPassCount,
    player1Score: player1ScoreAfter,
    player2Score: player2ScoreAfter,
    player1Rack: game.player1_rack,
    player2Rack: game.player2_rack,
