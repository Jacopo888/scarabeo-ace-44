...
import { boardToMap, mapToBoard } from './adapters'
...
describe('integration: scoring core vs calculateScore', () => {
  ...
  it('matches with bingo case', () => {
    ...
    const s1 = scoreMove(b, move).score
    const s2 = calculateScore({ tiles: move, existingBoard: boardToMap(b) })
    expect(s1).toBe(s2)
  })
})
