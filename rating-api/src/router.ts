...
export const registerRoutes = (app: Express) => {
  ...
  app.post('/rating/report', async (req, res, next) => {
    try {
      ...
      if (!player1Id || !player2Id || !winnerId || !mode) return res.status(400).json({ error: 'Missing fields' })
      if (!['blitz', 'rapid', 'async'].includes(mode)) return res.status(400).json({ error: 'Invalid mode' })
